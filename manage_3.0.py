...
class Manage:
	...
	def create_file(self, archive, content):
		with open(archive, 'w') as f:
			f.write(content)
			print("\033%s\033\tCriado com sucesso" %f.name)
			print("-"*100)
...
if __name__ == '__main__':
	...
	help = '''\n%s\n\033Script que facilita a criação de {Controllers, Views e Models} para o Framework\033\n%s\
	\n\n Uso básico: \033[ manage.py [-name NOME]]\033\n
	\n\t-help \t\tExibe o painel de ajuda\
	\n\t-name \t\tPassa o argumento seguido do nome dos arquivos:\033[ manage.py [-name NOME]\033\
	\n\t-table \t\tCaso o nome da tabela no banco de dados seja diferente do passado no argumento -name use esse argumento para passar o nome da tabela:\033[ manage.py [-name NOME] [-table NOME_TABELA]\033\
	\n\t-c \t\tPara criar apenas o controller\033[ manage.py [-name NOME] [-c]\033\
	\n\t-m \t\tPara criar apenas o model:\033[ manage.py [-name NOME] [-m]\033\
	\n\t-v \t\tPara criar apenas a view:\033[ manage.py [-name NOME] [-v]\033\
	\n\t-v-name \tPara escolher um nome específico para o arquivo phtml da view:\033[ manage.py [-name NOME] [-v-name NOME_VIEW]\033 (não coloque o '.phtml')\
	\n\n''' %('='*80, '='*80)
